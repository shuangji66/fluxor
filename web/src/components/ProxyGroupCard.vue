...
const getDelayClass = (delay?: number) => {
  if (delay === undefined) return 'bg-apple-input border-apple-border text-apple-text-muted hover:bg-accent hover:text-white hover:border-accent'
  if (delay === 0) return 'bg-apple-input border-apple-border text-apple-text-muted/60 animate-pulse'
  if (delay === -1) return 'bg-danger/10 border-danger/20 text-danger hover:bg-danger hover:text-white hover:border-danger'
  if (delay <= 200) return 'bg-success/10 border-success/20 text-success hover:bg-success hover:text-white hover:border-success'
  if (delay <= 500) return 'bg-warning/10 border-warning/20 text-warning hover:bg-warning hover:text-white hover:border-warning'
  return 'bg-danger/10 border-danger/20 text-danger hover:bg-danger hover:text-white hover:border-danger'
}
...
<template>
  <div ref="cardRef" class="bg-apple-card/50 rounded-lg border border-apple-border transition-all relative">
    <!-- 头部：展开时粘性，使用当前主题的卡片背景色 bg-apple-card/80 搭配模糊进行完美适配 -->
    <div
      class="px-4 sm:px-5 pt-4 sm:pt-5 pb-2 rounded-t-lg cursor-pointer select-none transition-shadow duration-200"
      :class="[
        expandedState[group.name]
          ? 'sticky top-0 z-10 bg-apple-card/80 backdrop-blur-md border-b border-apple-border shadow-none'
          : 'bg-transparent'
      ]"
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between gap-4">
          <div class="flex items-center gap-2.5 min-w-0">
            <ChevronForwardOutline class="w-3.5 h-3.5 text-apple-text-muted shrink-0 transform transition-transform duration-300" :class="{ 'rotate-90': expandedState[group.name] }" />
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                <span class="text-sm font-bold text-apple-text truncate">{{ group.name }}</span>
                <span class="px-2 py-0.5 text-[11px] font-extrabold bg-apple-bg text-apple-text-muted rounded-xs uppercase shrink-0">{{ group.type }}</span>
              </div>
              <div class="text-xs text-apple-text-muted mt-0.5 truncate">
          ...
          <div class="flex items-center gap-1.5">
            ...
            <button
              ...
              class="p-1.5 text-apple-text-muted hover:text-accent rounded-sm hover:bg-apple-border/50 transition-all shrink-0 active:scale-95"
            ...
            <button @click.stop="handleTestGroup" :disabled="isTesting" class="p-2 text-apple-text-muted hover:text-apple-text rounded-sm hover:bg-apple-border/50 transition-all shrink-0 active:scale-95" :title="t('proxies.test')">
    ...
    <div v-if="expandedState[group.name]" ref="gridRef" class="grid grid-cols-2 gap-2.5 px-4 sm:px-5 pb-4 sm:pb-5 pt-4 border-t border-apple-border">
      <div
      ...
      >
        ...
        <div v-if="allProxiesRaw[name]" class="flex justify-between items-center gap-1.5 mt-2.5 w-full select-none">
          <div class="flex items-center gap-1 min-w-0">
            <span class="bg-apple-bg text-apple-text-muted px-1 py-0.5 rounded-xs font-mono uppercase text-[9px] font-bold leading-none truncate">
              {{ allProxiesRaw[name].type || 'DIRECT' }}
            </span>
            <span v-if="allProxiesRaw[name].xudp" class="bg-success/10 text-success px-1 py-0.5 rounded-xs font-mono font-extrabold text-[9px] leading-none shrink-0" title="XUDP">X</span>
            <span v-else-if="allProxiesRaw[name].udp" class="bg-accent/10 text-accent px-1 py-0.5 rounded-xs font-mono font-extrabold text-[9px] leading-none shrink-0" title="UDP">U</span>
          </div>
          <div class="flex items-center gap-1 shrink-0">
            ...
            <span
              class="text-[10px] font-mono shrink-0 select-none px-1.5 py-0.5 rounded-sm leading-none text-center min-w-[32px] transition-all hover:scale-105 active:scale-95 border cursor-pointer"
        ...
        <div v-if="allProxiesRaw[name]" class="flex gap-[2px] w-full mt-2 h-1 overflow-hidden">
          ...
          <template v-else>
            <span class="flex-1 h-full bg-apple-border/50 rounded-sm"></span>
          </template>
        </div>
      </div>
    </div>
