...
<template>
  <div class="space-y-6 w-full">
    ...
    <div v-if="coreStatus.running" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative">
      ...
      <div
        class="bg-white dark:bg-[#1e293b] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-5 transition-all">
        ...
        <div class="flex items-center justify-between">
          <div>
            <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ t('config.ipv6_toggle')
              }}</label>
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <select v-model="configs.mode" @change="changeMode"
            class="px-3 py-2 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-accent outline-none">
        ...
        <div class="flex flex-col gap-1.5">
          <label class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ t('config.interface_name')
            }}</label>
          <div class="flex gap-2">
            <input type="text" v-model="configs['interface-name']" :placeholder="t('config.interface_name_placeholder')"
            ...
            <button @click="saveInterface"
              class="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-semibold rounded-lg transition-all">{{
      ...
      <div
        ...
        <div class="flex justify-between items-center border-t border-slate-100 dark:border-slate-800 pt-4">
          ...
          <button @click="savePorts"
            class="px-4 py-1.5 bg-accent hover:bg-accent-hover text-white text-xs font-semibold rounded-lg shadow-sm transition-all">{{
              t('config.save_ports') }}</button>
        </div>
      </div>
      ...
      <div
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="flex flex-col gap-1">
            ...
            <select v-model="configs.tun.stack"
              class="px-3 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-accent outline-none">
          ...
          <div class="flex flex-col gap-1">
            ...
            <input type="text" v-model="configs.tun.device"
              class="px-3 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-accent outline-none" />
          </div>
        </div>

        <div class="flex justify-end pt-2 border-t border-slate-100 dark:border-slate-800">
          <button @click="saveTun"
            class="px-4 py-1.5 bg-accent hover:bg-accent-hover text-white text-xs font-semibold rounded-lg shadow-sm transition-all">{{
              t('config.save_tun') }}</button>
        </div>
      </div>

      <div
        class="bg-white dark:bg-[#1e293b] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 lg:col-span-3 transition-all">
        <h4 class="font-bold text-sm border-b border-slate-100 dark:border-slate-800 pb-3 flex items-center gap-2">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
          <button @click="handleReloadConfig"
            class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-semibold rounded-xl text-slate-700 dark:text-slate-200 transition-all border border-slate-200/20">
            {{ t('config.reload') }}
          </button>
          <button @click="handleRestartCore"
            class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-semibold rounded-xl text-slate-700 dark:text-slate-200 transition-all border border-slate-200/20">
            {{ t('config.restart') }}
          </button>
          <button @click="handleFlushFakeIP"
            class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-semibold rounded-xl text-slate-700 dark:text-slate-200 transition-all border border-slate-200/20">
            {{ t('config.flush_fakeip') }}
          </button>
          <button @click="handleFlushDNS"
            class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-semibold rounded-xl text-slate-700 dark:text-slate-200 transition-all border border-slate-200/20">
            {{ t('config.flush_dns') }}
          </button>
          <button @click="handleUpdateGeo"
            class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-semibold rounded-xl text-slate-700 dark:text-slate-200 transition-all border border-slate-200/20 col-span-2 lg:col-span-1">
            {{ t('config.update_geo') }}
          </button>
        </div>
      </div>
      ...
      <div
        class="bg-white dark:bg-[#1e293b] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 md:col-span-2 lg:col-span-3 transition-all">
        <h4 class="font-bold text-sm border-b border-slate-100 dark:border-slate-800 pb-3 flex items-center gap-2">
        ...
        <div class="flex gap-2">
          <input type="text" v-model="dnsQuery.name" :placeholder="t('config.dns_placeholder')"
            ...
            class="flex-1 px-4 py-2 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-accent outline-none" />
          ...
          <button @click="handleDNSQuery" :disabled="dnsQuery.loading"
            class="px-5 py-2 bg-accent hover:bg-accent-hover text-white text-xs font-semibold rounded-lg shadow-sm transition-all">
            {{ dnsQuery.loading ? t('config.dns_querying') : t('config.dns_query_btn') }}
          </button>
        </div>

        <pre v-if="dnsQuery.result"
          class="p-4 bg-slate-950 text-emerald-400 font-mono text-xs rounded-xl overflow-x-auto max-h-40 border border-slate-800">{{ dnsQuery.result }}</pre>
      </div>
    </div>

    <!-- 2. 内核状态与管理面板（始终显示在最下方） -->
    <div
      class="bg-white dark:bg-[#1e293b] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6 transition-all">
      <div class="flex items-center gap-4">
        <span class="w-4.5 h-4.5 rounded-full flex shrink-0"
          :class="coreStatus.loading ? 'bg-slate-400 animate-pulse' : (coreStatus.running ? 'bg-success shadow-lg shadow-success/40' : 'bg-red-500 shadow-lg shadow-red-500/40')"></span>
        <div>
          <div class="flex items-center gap-2">
            <h3 class="font-bold text-base">{{ coreStatus.running ? t('config.core_running') : t('config.core_stopped')
              }}</h3>
            <span v-if="coreStatus.running && stats.coreVersion !== '未知' && stats.coreVersion !== '加载中...'"
              class="px-2 py-0.5 text-[10px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-md">
              {{ coreVersion }}
            </span>
          </div>
      ...
      <div class="flex gap-2">
        <button v-if="!coreStatus.running" @click="handleStartCore" :disabled="coreStatus.loading"
          class="px-4 py-2 bg-success text-white text-xs font-semibold rounded-lg shadow-md shadow-success/15 hover:shadow-success/25 transition-all">
          {{ t('config.start_core') }}
        </button>
        <template v-else>
          <button @click="handleStopCore" :disabled="coreStatus.loading"
            class="px-4 py-2 bg-red-500 text-white text-xs font-semibold rounded-lg shadow-md shadow-red-500/15 hover:shadow-red-500/25 transition-all">
            {{ t('config.stop_core') }}
          </button>
          <button @click="handleRestartCore" :disabled="coreStatus.loading"
            class="px-4 py-2 bg-amber-500 text-white text-xs font-semibold rounded-lg shadow-md shadow-amber-500/15 hover:shadow-amber-500/25 transition-all">
            {{ t('config.restart') }}
          </button>
        </template>
        <button @click="handleUpgradeCore" :disabled="isUpgrading"
          class="px-4 py-2 bg-accent text-white text-xs font-semibold rounded-lg shadow-md shadow-accent/15 hover:shadow-accent/25 transition-all flex items-center gap-1">
