...
<template>
  <div class="flex flex-col flex-1 min-h-0 gap-4 h-full">
    <div class="glass-medium shadow-none px-6 py-3 md:py-0 rounded-lg border border-apple-border flex flex-wrap gap-4 items-center justify-between transition-all shrink-0 h-auto min-h-[56px] md:h-[56px]">
      <div class="flex flex-wrap items-center gap-x-4 gap-y-2 flex-1 md:flex-initial">
        ...
        <div class="flex bg-apple-input rounded-full p-0.5 transition-all border border-apple-border relative select-none w-full sm:w-64">
          ...
          ></div>
          <button @click="activeTab = 'rules'" class="flex-1 px-4 py-1.5 text-xs font-semibold rounded-full transition-all duration-200 active:scale-95 z-10 text-center" :class="activeTab === 'rules' ? 'text-white' : 'text-apple-text-muted hover:text-apple-text'">
            {{ t('rules.rules_tab') }} ({{ rules.length }})
          </button>
          <button @click="activeTab = 'providers'" class="flex-1 px-4 py-1.5 text-xs font-semibold rounded-full transition-all duration-200 active:scale-95 z-10 text-center" :class="activeTab === 'providers' ? 'text-white' : 'text-apple-text-muted hover:text-apple-text'">
      ...
      <div class="flex items-center gap-3 flex-1 justify-end min-w-[280px] sm:min-w-0 flex-nowrap">
        <input type="text" v-model="searchText" :placeholder="t('rules.search_placeholder')" class="w-full sm:w-60 px-4 py-1.5 text-xs rounded-full border border-apple-border bg-apple-input text-apple-text focus:ring-1 focus:ring-accent outline-none placeholder-apple-text-muted/50" />
        
        <div class="flex gap-2 shrink-0">
          <button v-if="activeTab === 'providers'" @click="handleUpdateAllProviders" :disabled="isUpdatingAll" class="px-4 py-1.5 bg-accent hover:bg-accent-hover text-white text-xs font-semibold rounded-full shadow-none transition-all flex items-center gap-1.5 disabled:cursor-not-allowed whitespace-nowrap active:scale-[0.95]">
    ...
    <!-- 统一大内容卡片 -->
    <div class="flex-1 min-h-0 glass-medium shadow-none rounded-lg border border-apple-border flex flex-col overflow-hidden">
      <!-- 规则列表 -->
      <div v-if="activeTab === 'rules'" class="flex-1 min-h-0 overflow-y-auto pr-1">
        <div v-if="isLoadingRules && rules.length === 0" class="p-8 text-center text-apple-text-muted text-sm">
          {{ t('common.loading') }}
        </div>
        <template v-else>
          <div v-if="filteredRules.length === 0" class="p-8 text-center text-apple-text-muted text-sm">
            {{ t('rules.no_rules_found') }}
          </div>
          <div v-else class="divide-y divide-apple-border">
            <div v-for="(rule, idx) in visibleRules" :key="idx" class="px-5 py-3.5 flex items-center justify-between gap-4 hover:bg-apple-card transition-colors duration-200">
              <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                  <span class="px-2 py-0.5 text-[10px] font-bold uppercase rounded-xs tracking-wide shrink-0" :class="{
                    'bg-success/10 text-success': rule.type === 'IP-CIDR' || rule.type === 'IP-CIDR6',
                    'bg-warning/10 text-warning': rule.type === 'DOMAIN-SUFFIX' || rule.type === 'DOMAIN-KEYWORD',
                    'bg-accent/10 text-accent': rule.type === 'DOMAIN',
                    'bg-apple-border text-apple-text-muted': rule.type === 'MATCH'
                  }">
                    {{ rule.type }}
                  </span>
                  <span class="text-sm font-medium text-apple-text break-all select-all">{{ rule.payload }}</span>
                </div>
                <div class="text-xs text-apple-text-muted mt-1">
                  {{ t('rules.proxy') }}: <span class="font-semibold text-accent">{{ rule.proxy }}</span>
                </div>
              </div>

              <div class="shrink-0">
                <button @click="handleToggleRule(rule)" class="w-10 h-6 flex items-center rounded-full p-0.5 transition-all outline-none duration-200 border" :class="rule.enabled ? 'bg-accent border-transparent justify-end' : 'switch-bg-off border-apple-border/60 justify-start'">
                  <span class="w-5 h-5 rounded-full bg-white shadow-[0_1.5px_3px_rgba(0,0,0,0.2),0_1px_1px_rgba(0,0,0,0.05)] border border-slate-200/30"></span>
                </button>
              </div>
            </div>
            <div v-if="filteredRules.length > displayLimit" class="py-3 text-center text-xs text-apple-text-muted flex items-center justify-center gap-1.5 border-t border-apple-border bg-apple-card/30">
              <div class="w-3.5 h-3.5 border border-apple-border !border-t-accent rounded-full animate-spin"></div>
      ...
      <div v-if="activeTab === 'providers'" class="flex-1 min-h-0 overflow-y-auto pr-1">
        <div v-if="isLoadingProviders && providers.length === 0" class="p-8 text-center text-apple-text-muted text-sm">
          {{ t('common.loading') }}
        </div>
        <template v-else>
          <div v-if="filteredProviders.length === 0" class="p-8 text-center text-apple-text-muted text-sm">
            {{ t('rules.no_providers') }}
          </div>
          <div v-else class="divide-y divide-apple-border">
            <div v-for="p in filteredProviders" :key="p.name" class="p-5 flex items-center justify-between gap-4 hover:bg-apple-card transition-colors relative overflow-hidden">
              <!-- 规则更新局部加载遮罩 -->
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <strong class="text-sm font-bold text-apple-text break-all">{{ p.name }}</strong>
                  <span class="px-1.5 py-0.5 text-[9px] font-semibold bg-apple-bg text-apple-text-muted rounded-xs uppercase">{{ p.type }}</span>
                </div>
                <div class="text-xs text-apple-text-muted mt-1.5 space-x-4">
                  <span>{{ t('rules.rule_count') }}: <strong class="text-apple-text">{{ p.ruleCount }}</strong></span>
                  <span>{{ t('rules.updated_at') }}: <span class="text-apple-text-muted">{{ formatDate(p.updatedAt) }}</span></span>
                </div>
              </div>

              <div class="shrink-0">
                <button @click="handleUpdateProvider(p.name)" :disabled="isUpdating[p.name]" class="p-2 hover:bg-apple-border/50 text-apple-text-muted hover:text-apple-text rounded-sm active:scale-95 transition-all">
