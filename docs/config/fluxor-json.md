...
Fluxor 的设计核心之一是不引入复杂的数据库系统。所有的全局参数、机场订阅链接及拉取元数据，都持久化存储在磁盘中一个单一的 JSON 文件中。
...
* **`proxy_port`**：混合代理端口（Mixed Port），该端口同时支持 HTTP 和 SOCKS5 代理协议，默认端口为 `7890`。
* **`tproxy_port`**：透明代理网关端口（TProxy Port），专门供 nftables 防火墙重定向劫持流量使用，默认端口为 `7898`。
